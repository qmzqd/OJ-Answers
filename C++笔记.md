...
## 整数
- `short`: 占用2字节的存储空间，通常有16位整数。
- `int`: 占用4字节的存储空间，是最常见的整数类型。
- `long long`: 占用8字节的存储空间，提供更大的整数范围。

## 浮点数
- `float`: 单精度浮点数，占用4字节，适用于需要小数的计算。
...
## 布尔
- `bool`: 存储逻辑值`true`或`false`，通常占用1字节。
...
## 二进制转换
- 十六进制转二进制：1位十六进制数转换为4位二进制数。
- 八进制转二进制：1位八进制数转换为3位二进制数。

# 位运算

- 按位与(`&`)：参与运算的两个位都为1时，结果为1。
- 按位或(`|`)：参与运算的两个位中至少有一个为1时，结果为1。
